use crate::{
    ExtractedModules, FileType, analyzer::fingerprints::fingerprinting::FingerprintPattern,
};

use regex::Regex;
use std::collections::hash_set::HashSet;

pub fn run(data: &ExtractedModules, patterns: &Vec<FingerprintPattern>) -> Vec<String> {
    let mut fetched_libs: Vec<String> = Vec::new();

    if data.file_type == FileType::ELF {
    ...
    } else if data.file_type == FileType::PE {
